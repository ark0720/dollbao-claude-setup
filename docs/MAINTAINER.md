...
- **例子：** （目前 repo 無 active 兩層索引型 skill；模板與 digest SOP 保留供未來使用）
- **寫法：**
...
- **digest 同步 SOP 見 §9**（章節保留供參考；目前無 active skill 需同步）

### 2c. BQ 查詢型（BigQuery-backed）
- **特徵：** Source of truth 在 BigQuery，skill 給 AI 看 schema + 範例 SQL，觸發時即時組 query
- **例子：** `dollbao-calendar`（逗寶曆）
- **寫法：**
  - body 列 project / dataset / table（或 view）路徑
  - 列完整 schema（欄位名 + 型別 + 說明）
  - 列 3-5 個範例 SQL（涵蓋常見問題型態）
  - 權限備註：失敗時提示 `gcloud auth login`

#### 2c-i. 歸屬判斷 — manifest 必須 ⊆ IAM 授權範圍 ⚠️

⚠️ **BQ 查詢型 skill 放哪個 manifest，由 underlying view / table 的 IAM 授權範圍決定，不是「BQ vs 非 BQ」。**

| underlying view / table 的 IAM grant 對象 | skill 該放哪 |
|---|---|
| 全員 Google 群組（all-employees）| `manifest/common.json` |
| 特定部門群組（如 `marketing@dollbao.com`、`purchasing@dollbao.com`）| `manifest/dept-{key}.json` |
| 個別人員 | 不該變成 repo 內 skill，留個人（§5） |

**兩種失敗模式：**
- ❌ **manifest 比 IAM 寬：** 沒權限的人裝了卻跑不通 → 用戶卡關、可能找 workaround 繞權限
- ❌ **manifest 比 IAM 窄：** 有權限的人沒裝到 → 功能浪費

**新增 BQ 查詢型 skill 的標準流程：**

1. **先確認 view / table 已 IAM 授權給哪些群組：**
   ```powershell
   bq show --iam-policy {project}:{dataset}.{table}
   # 或對 view：bq show --iam-policy --view {project}:{dataset}.{view}
   ```
2. **依授權對象決定 manifest 歸屬**（依上表）
3. **若想擴大 manifest 範圍，必須*先*改 IAM grants 再改 manifest** — 不要 manifest 先到位、IAM 後補（會出現一批人裝了但跑不通）
4. 走 §4 三道閘（明確 trigger → double-check 位置 → hard stop）

**例：** `dollbao-calendar` 查的是逗寶曆 table，授權對象是全員群組 → 進 `common.json` ✅。未來若有「庫存成本 BQ skill」、underlying view 只授權給採購部群組 → 必須進 `dept-purchasing.json`，不能進 common（否則行銷部新人裝完跑會被 IAM 擋）。
...
4. **scaffold** `skills/X/SKILL.md` — 按型態套對應 template（BQ 查詢型可參考 `dollbao-calendar`；兩層索引型目前無 active 範例，必要時參考 §9 SOP）；若是從草稿升級則跳過此步
...
⚠️ **目前無 active 兩層索引型 skill；本節僅作 reference**，未來若新增規章 / SOP 類索引型 skill 時可參考流程。觸發句「規章 Doc 有更新」在現行 repo 不再有對應動作。
...
- ❌ 把 BQ 查詢類 skill 的 manifest 歸屬與 underlying view / table 的 **IAM 授權範圍不對齊**（詳見 §2c-i）：
  - 把「只給特定部門群組讀」的 view skill 放進 `common.json` → 全員裝完卻沒權限跑
  - 把「全員可讀」的 view skill 放進 `dept-*.json` → 有權限的人沒裝到
  - **不**是「BQ 查詢類 skill 一律不能進 common」 — `dollbao-calendar` 就是 BQ + common 的合法例子，因為逗寶曆 table 對全員開放
- ❌ **省略 §4 三道閘任何一道**：
