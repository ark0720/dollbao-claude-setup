...
- **資料 source of truth：** BigQuery table `dollbao-data-center.Ref.reportYearMonthWeek`
...
| 項目 | 值 |
|---|---|
| GCP Project (資料) | `dollbao-data-center` |
| Dataset | `Ref` |
| Table | `reportYearMonthWeek` |
| Billing project (跑 query) | `dollbao-gws-cli` |
| 涵蓋範圍 | RY19 ~ RY29（6573 列，~18 年） |
| 授權 | table-level `roles/bigquery.dataViewer` → `domain:dollbao.com.tw`；project-level `roles/bigquery.jobUser` on `dollbao-gws-cli` → `domain:dollbao.com.tw` |
...
SELECT
  ...
  CASE
    ...
    ELSE 'RQ4'
  END AS RQ
FROM `dollbao-data-center.Ref.reportYearMonthWeek`
...
```sql
WITH this_ry AS (
  SELECT reportYear FROM `dollbao-data-center.Ref.reportYearMonthWeek`
  WHERE date = CURRENT_DATE('Asia/Taipei')
)
SELECT
  CONCAT('RY', LPAD(CAST(reportYear AS STRING), 2, '0')) AS RY,
...
LEFT JOIN `dollbao-data-center.Ref.reportYearMonthWeek` r
